"""Integration test fixtures: real database, Redis, and local task runner.

The test stack mirrors dev/prod as closely as possible:
- **PostgreSQL** on port 5433 (``docker-compose.test.yml``)
- **Redis** on port 6380 for rate limiting and usage quotas
...
async def db_session() -> AsyncGenerator[AsyncSession, None]:
    ...
    await session.execute(
        text(
            "TRUNCATE local_jobs, eval_monitors, session_scores, trace_scores, eval_runs, spans, traces, api_keys, memberships, users, projects, subscriptions, usage_records, organizations CASCADE"
        )
    )
    await session.commit()
    await session.close()
